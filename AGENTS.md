...
- 仓库内包含静态前端产物 `frontend-dist`，但不包含完整前端源码；前端源码参考 README 指向的外部项目。
...
- `frontend-dist` 是静态前端产物目录。修改 `frontend-dist/js/nav-hotfix.js`、`frontend-dist/css/nav-hotfix.css` 等热修复文件时，应保持对应 `.gz` 文件同步，并运行导航热修复相关测试。
