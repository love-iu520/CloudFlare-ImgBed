# CloudFlare-ImgBed 项目协作规则

本文件只补充或覆盖本项目特有规则；未说明事项继承当前生效的全局 `AGENTS.md`。
...
- 前端界面、文案和局部交互的 Source of Truth 位于独立前端仓。只有用户明确要求生产构建、同步部署产物、发布或等价交付动作时，才构建并修改 `dist` / 本仓 `frontend-dist/`；同一源码状态不重复构建。
- `frontend-dist/` 仅保存部署产物。直接热修复产物只限任务明确要求的场景，并同步对应 `.gz` 文件；完整流程见 `docs/FRONTEND_DIST_SYNC.md`。
...
是否验证及验证范围继承全局规则；需要验证时，从下表选择直接命中目标的最低必要入口。
...
| 前端源码（仅源码开发） | 在前端源码仓运行直接命中目标的测试 | 无直接测试的纯界面修改由开发模式手动验证，不触发生产构建 |
